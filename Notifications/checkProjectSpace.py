...
DBNAME='/home/scipionuser/EMadmin/EMadmin/src/db.sqlite3'
TWOWEEKS=1209600
...
def _print(list, msg):
    return 
    # comment avobe line for testing
    print msg
    for item in list:
        print item
    return
...
def getProjecs():
    # get list of projects in OffloadData
    # anything starting with a date is a project, e.g.
    # 2018_01_21_pepe
    ...
    _print(projectList, "initial list")  # debug
    # keep projects older than 2 weeks
    projectList = [item for item in projectList if item.isOldProject()]
    # remove projects in tabu list
    projectList = [item for item in projectList if item.isNotInTabuList()]
    _print(projectList, "final list")  # debug
    return projectList
...
def sendEmails(rows):
    ...
    We would appreciate if you send and email to the facility staff 
    to let us know that your file are already transfered.
    (rarranz@cnb.csic.es; fjchichon@cnb.csic.es; rnunez@cib.csic.es)

    Yours faithfully.

        CNB-CIB CryoEM Facility Staff"""
    ...
    for row in rows:
        ...
        # for historical reasons admin has a fake name redirect to 
        # f.J. chichon
        if row[1] == 'administrator@gmail.com':
           toAddress='fjchichon@cnb.csic.es'
        else:
          toAddress=row[1]
        print toAddress
        print msg % (row[0], row[2], date), row[3]
        sendEMail(toAddress, 'project %s' % row[0], msg % (row[0], row[2], str(today + week)) )
        #next line if for testing
        # sendEMail('locwiki@gmail.com', 'project %s' % row[0], msg % (row[0], row[2], date))


# send email complaining

# get list of proejcts older than 14 days
projectList = getProjecs()
# get emails associated to those accounts
rows = getEmails(projectList)
# send emails using local smtp
sendEmails(rows)
