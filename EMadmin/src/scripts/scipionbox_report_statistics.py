...
def getCtfStats(ctfSet):
    ...
    astigmatimList = []
    averageResolution  = 0
    averageAstigmatism = 0
    numberMovies = len(ctfSet)
    for ctf in ctfSet:
        resolution = ctf.getResolution()
        defocusU   = ctf.getDefocusU()
        defocusV = defocusU - ctf.getDefocusV()
        # remove inf and nan
        if resolution < 1000 and defocusU < 100000 and defocusV < 100000\
           and resolution > 0 and defocusU > -100000 and defocusV > -100000:
            resolutionList.append(resolution)
            defocusUList.append(defocusU)
            astigmatimList.append(defocusV)
            averageResolution += resolution
            averageAstigmatism += defocusV

    averageResolution /= len(defocusUList)
    averageAstigmatism /= len(defocusUList)
    return resolutionList, defocusUList, astigmatimList, averageResolution, averageAstigmatism,numberMovies
...
def getCTFData(projectName):
    ...
    for run in project.getRuns():
        for outputName, output in run.iterOutputAttributes(SetOfCTF):
            resolutionData, defocusData, astigmaticData, averageResolution, averageAstigmatism, \
            numberMovies = getCtfStats(output)
            # return the first set of CTFs found
            return {"resolutionData": resolutionData,
                    "defocusData": defocusData,
                    "astigmaticData": astigmaticData,
                    "averageResolution": averageResolution,
                    "averageAstigmatism": averageAstigmatism,
...
def main(projectName):
    ...
    ctfDict = getCTFData(projectName)
